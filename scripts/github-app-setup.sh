...
#   hub/.env                                 — GITHUB_APP_ID, GITHUB_WEBHOOK_SECRET
#
# Usage:
#   scripts/github-app-setup.sh path/to/app-manifest.json
#   scripts/github-app-setup.sh path/to/key.pem --app-id 123456 [--webhook-secret SECRET]
#
# The JSON form is what the App-manifest flow returns (id + pem + webhook_secret
# in one blob). The .pem form is the plain "Generate a private key" download,
# where the App ID lives on the App's settings page and the webhook secret is
...
[[ -n "$SOURCE" ]] || { echo "usage: $0 <manifest.json|key.pem> [--app-id N] [--webhook-secret S]" >&2; exit 2; }
...
python3 - "$ENV_PATH" "$APP_ID" "$WEBHOOK_SECRET" <<'PY'
import pathlib, sys

path, app_id, secret = pathlib.Path(sys.argv[1]), sys.argv[2], sys.argv[3]
header = [
